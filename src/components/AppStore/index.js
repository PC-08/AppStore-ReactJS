...
class AppStore extends Component {
  state = {activeTab: tabsList[0].tabId, userInput: ''}
  ...
  render() {
    const {userInput, activeTab} = this.state

    const filteredData = appsList.filter(
      eachCat =>
        eachCat.category === activeTab &&
        eachCat.appName
          .toLocaleLowerCase()
    ...
    return (
      <div className="app-store-bg">
        ...
        <div className="app-store-searchContainer">
          <input
            placeholder="Search"
            value={userInput}
            onChange={this.onUserEnter}
            className="app-store-input"
            type="search"
          />{' '}
          <img
            className="app-store-search-img"
            alt="search icon"
            src="https://assets.ccbp.in/frontend/react-js/app-store/app-store-search-img.png"
          />
        </div>
        <div>
          <ul className="app-store-tab-list">
            {tabsList.map(eachTabItem => (
              <TabItem
                key={eachTabItem.tabId}
                tabId={eachTabItem.tabId}
                displayText={eachTabItem.displayText}
                changeTab={this.changeTab}
                isActiveTab={eachTabItem.tabId === activeTab}
              />
            ))}
          </ul>
        </div>
        ...
        <div>
          <ul className="app-store-app-list">
